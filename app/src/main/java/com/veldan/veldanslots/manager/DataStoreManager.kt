package com.veldan.veldanslots.manager
...
object DataStoreManager {
    ...
    object Balance: DataStoreElement<Long> {
        override suspend fun collect(block: suspend (Long) -> Unit) {
            game.activity.dataStore.data.collect { block(it[BALANCE_KEY] ?: 1_000L) }
        }

        override suspend fun update(block: suspend (Long) -> Long) {
            game.activity.dataStore.edit { it[BALANCE_KEY] = block(it[BALANCE_KEY] ?: 1_000L) }
        }
